...
import { AccountPayable } from '@/types/api';
...
export default function AccountsPayablePage() {
  ...
  async function loadAccounts() {
    try {
      setLoading(true);
      const data = await financeService.getAllAccountsPayable();
  ...
  // Criar conta
  async function onCreateSubmit(data: AccountPayableFormData) {
    try {
  ...
  // Renderizar ações
  function renderActions(account: AccountPayable) {
    if (account.status === 'paid') {
      return (
        <span className="text-sm text-gray-600">
          Pago em {account.paid_date ? formatDate(account.paid_date) : '-'}
        </span>
      );
    }
  ...
  return (
    <div className="p-8">
      ...
      <Dialog open={confirmPayModalOpen} onOpenChange={setConfirmPayModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Confirmar Pagamento</DialogTitle>
            <DialogDescription>
              Ao marcar como paga, esta ação criará automaticamente uma{' '}
              <strong>SAÍDA no caixa</strong>. Deseja continuar?
            </DialogDescription>
          </DialogHeader>
          {selectedAccount && (
      ...
      <Dialog open={confirmDeleteModalOpen} onOpenChange={setConfirmDeleteModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Tem certeza que deseja remover esta conta a pagar? Esta ação não pode ser
              desfeita.
            </DialogDescription>
          </DialogHeader>
          {selectedAccount && (
